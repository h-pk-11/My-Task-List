...
export function activeTabsEvent() {
  ...
  for (let i = 0; i < 3; i++) {
    appSideBar__taskItems[i].onclick = function () {
      setTimeout(() => {
        handler.trigger('change view mode', appSideBar__taskItems[i].dataset.viewMode);
    ...
    item.onclick = function () {
      ...
      setTimeout(() => {
        if (!isProjectNameEditMode) {
          ...
          handler.trigger('change view mode', item.dataset.viewMode, item.dataset.projectId);
  ...
  appSideBar__items.forEach((item) => {
    ...
    if (removeBtn) {
      removeBtn.onclick = function (e) {
        ...
        const numberOfProjects = handler.trigger('delete project', item.dataset.projectId);
